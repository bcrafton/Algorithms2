...
void WordList::loadWordList(string wordListFileName) throw(fileOpenError)
{
	...
	{
		if((*itr).length() > maxWordLength)
		{
			maxWordLength = (*itr).length();
		}
		itr++;
	}
...
/*
	calls the heap sort function
*/
void WordList::heapSort()
{
	Heap<string> h(wordList);
	wordList = h.heapSortAscending();
}
...
int WordList::getMaxWordLength() const
{
	return maxWordLength;
}

Hashtable<string> WordList::makeHashtable()
{
	Hashtable<string> h(10000);

	const int wordListSize = wordList.size();
	for(int counter = 0; counter < wordListSize; counter++)
	{
		h.add(wordList[counter]);
	}
	/*
	vector<string>::iterator itr;
	itr = wordList.begin();
